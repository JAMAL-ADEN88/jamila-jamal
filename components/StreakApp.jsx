...
export default function StreakApp() {
  ...
  // ── Load profile & couple when user logs in ────────────────────────────────
  useEffect(() => {
    if (user) {
      loadProfile();
    }
  ...
  async function loadProfile() {
    ...
    if (data) {
      ...
      const { data: newProfile } = await supabase
        .from("profiles")
        .insert({ id: user.id, name: user.user_metadata?.name || email.split("@")[0], invite_code: generateCode() })
        .select()
        .single();
  ...
  async function createCouple() {
    ...
    const { data: newCouple, error } = await supabase
      ...
      .select()
      .single();
    if (error) { setCoupleError("Error creating couple"); setCoupleLoading(false); return; }
    // Update profile with couple_id
    await supabase.from("profiles").update({ couple_id: newCouple.id }).eq("id", user.id);
    setProfile(p => ({ ...p, couple_id: newCouple.id }));
    setCouple(newCouple);
  ...
  async function joinCouple() {
    ...
    if (!partner.couple_id) { setCoupleError("Partner hasn't created a couple yet. Ask them to create one first."); setCoupleLoading(false); return; }
  ...
  async function confirmPeace() {
    ...
    if (bothConfirmed && isNewDay) {
      const newStreak = couple.streak + 1;
      updates.streak = newStreak;
      updates.total_peaceful = couple.total_peaceful + 1;
      updates.confirmed_by = [];
      if (newStreak > couple.longest_streak) updates.longest_streak = newStreak;
      setCelebrating(true);
  ...
  async function resetStreak() {
    await supabase.from("couples").update({
      streak: 0, confirmed_by: [], last_confirmed: null
    }).eq("id", couple.id);
    loadCouple(couple.id);
    setShowReset(false);
        ...
        <div className="auth-wrap">
          <div className="auth-card glass">
            ...
            <div className="auth-tabs">
              ...
              <button className={`auth-tab ${authMode === "signup" ? "active" : ""}`} onClick={() => { setAuthMode("signup"); setAuthError(""); }}>Create Account</button>
            </div>

            {authMode === "signup" && (
              <input className="auth-input" placeholder="Your name (Jamila or Jamal)"
              ...
              <div className="setup-block">
                ...
                {profile?.invite_code && (
                  <div className="invite-code-box">
                    ...
                    <p className="invite-hint">Share this with your partner</p>
                  </div>
                )}
              </div>
    ...
    <>
      ...
      <div className="app">
        ...
        <header className="app-header">
          <span className="brand" onClick={handleLogout} title="Sign out">J / J</span>
          <div className="header-mid">
            <span className="couple-names">
              {profile?.name || "You"} & {partnerProfile?.name || "Partner"}
            </span>
          </div>
        ...
        {activeTab === "home" && (
          <main className="tab">
            <div className="quote-bar glass">
              <p className="quote fade-in" key={quoteIdx}>"{QUOTES[quoteIdx]}"</p>
            </div>
            ...
            <div className={`streak-card glass ${celebrating ? "glow" : ""}`}>
              ...
              <div className="stats-row">
                ...
                <div className="sdiv" />
                <div className="stat">
                  <span className="sv">{partnerConfirmed ? "✅" : "⏳"}</span>
                  <span className="sk">Partner</span>
                </div>
              </div>
            </div>
            ...
            <div className="daily glass">
              ...
              <div className="check-btns">
                <button className={`btn-confirm ${todayConfirmed ? "confirmed" : ""}`}
                  onClick={confirmPeace} disabled={todayConfirmed}>
                  {todayConfirmed ? "✅ Confirmed!" : "✨ Yes, we had peace"}
                </button>
                <button className="btn-fight" onClick={() => setShowReset(true)}>
                  😔 We had a fight
                </button>
              </div>
              {todayConfirmed && !partnerConfirmed && (
                <p className="waiting-msg">Waiting for {partnerProfile?.name || "your partner"} to confirm… 💛</p>
              )}
              {todayConfirmed && partnerConfirmed && (
                <p className="waiting-msg">Both confirmed! Streak updated 🎉</p>
              )}
            </div>
            ...
            {!partnerProfile && (
              <div className="invite-remind glass">
                ...
                <span className="invite-code">{profile?.invite_code}</span>
                <p className="invite-hint">They enter this code when signing up</p>
              </div>
            )}
        ...
        {activeTab === "mood" && (
          <main className="tab">
            ...
            <div className="mood-grid glass">
              {[
                { e: "😌", l: "Peaceful" }, { e: "💛", l: "Loving" },
                { e: "😔", l: "Sad" }, { e: "😤", l: "Frustrated" },
                { e: "🥰", l: "Adoring" }, { e: "😴", l: "Tired" },
                { e: "🌟", l: "Grateful" }, { e: "🤗", l: "Need a Hug" },
              ].map(m => (
                <button key={m.l} className={`mood-btn ${mood === m.l ? "mood-active" : ""}`}
                  onClick={() => setMood(m.l)}>
...
:root{
  --gold:#d4af37;--gold2:#f5c842;--brown:#c97d4e;
  --bg:#0a0705;--s:rgba(255,255,255,0.04);--b:rgba(212,175,55,0.15);
  --t:#f2e8d5;--tm:rgba(242,232,213,0.5);--r:20px;
...
.auth-error{color:#e08080;font-size:.85rem;text-align:center}
...
.btn-secondary{background:transparent;border:1px solid var(--b);color:var(--t);font-family:var(--fb);border-radius:50px;padding:.9rem 2rem;font-size:.95rem;cursor:pointer;transition:background .2s,border-color .2s;width:100%}
...
@media(max-width:400px){
  .mood-grid{gap:.5rem}
  .mood-btn{padding:.65rem .3rem;font-size:1.2rem}
